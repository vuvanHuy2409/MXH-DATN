...
Route::middleware('auth')->group(function () {
    ...
    // Admin Dashboard - Chỉ email huyberr@gmail.com được vào
    // Admin Dashboard
    Route::get('/admin', [\App\Http\Controllers\Admin\DashboardController::class, 'index'])->name('admin.dashboard')->middleware('admin.only');
    ...
    Route::prefix('admin/users')->name('admin.users.')->middleware('admin.only')->group(function () {
        Route::get('/', [\App\Http\Controllers\Admin\UserManagementController::class, 'index'])->name('index');
        Route::get('/flagged', [\App\Http\Controllers\Admin\UserManagementController::class, 'flagged'])->name('flagged');
        Route::get('/import', [\App\Http\Controllers\Admin\UserManagementController::class, 'importIndex'])->name('import.index');
        Route::get('/import/template/student', [\App\Http\Controllers\Admin\UserManagementController::class, 'downloadStudentTemplate'])->name('import.template.student');
        Route::get('/import/template/teacher', [\App\Http\Controllers\Admin\UserManagementController::class, 'downloadTeacherTemplate'])->name('import.template.teacher');
        Route::post('/import/single', [\App\Http\Controllers\Admin\UserManagementController::class, 'importSingle'])->name('import.single');
        Route::post('/import/bulk', [\App\Http\Controllers\Admin\UserManagementController::class, 'importBulk'])->name('import.bulk');
        Route::post('/{id}/toggle-flag', [\App\Http\Controllers\Admin\UserManagementController::class, 'toggleFlag'])->name('toggle-flag');
        Route::delete('/{id}', [\App\Http\Controllers\Admin\UserManagementController::class, 'destroy'])->name('destroy');
    });

    // Cài đặt hệ thống (Admin)
    Route::prefix('admin/settings')->name('admin.settings.')->middleware('admin.only')->group(function () {
        ...
        Route::post('/test-connection', [\App\Http\Controllers\Admin\SystemSettingController::class, 'testConnection'])->name('test');
    });

    // Nút đăng xuất dùng chung
    Route::post('/logout', [AuthController::class, 'logout'])->name('logout');
    // Các route chỉ dành cho người dùng thường (Admin bị chặn và redirect về dashboard)
